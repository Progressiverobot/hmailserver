...
namespace hMailServer.Administrator
{
    public partial class formMain : Form, IMainForm
    {
        ...
        public formMain(hMailServer.Application app, string serverHost)
        {
            ...
            // Server features (hMailServer.INI) editor; available when
            // running on the server itself.
            var menuItemServerFeatures = new ToolStripMenuItem("Server &features...");
            menuItemServerFeatures.Click += delegate
            {
                using (var dialog = new Dialogs.formServerFeatures())
                    dialog.ShowDialog(this);
            };

            int insertIndex = menuItemFile.DropDownItems.IndexOf(toolStripSeparator1);
            if (insertIndex < 0)
        ...
        private void CreateMainNodes()
        {
            ...
            AddNode(null, new NodeDashboard(), 1);
