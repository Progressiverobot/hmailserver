...
namespace hMailServer.Administrator
{
   public partial class formConnect : Form
   {
      ...
      public formConnect()
      {
         ...
         // Two-factor authentication setup entry point.
         LinkLabel linkTwoFactor = new LinkLabel();
         linkTwoFactor.Text = "Two-factor authentication...";
         linkTwoFactor.AutoSize = true;
         linkTwoFactor.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
         linkTwoFactor.Location = new System.Drawing.Point(12, ClientSize.Height - 24);
         linkTwoFactor.LinkClicked += delegate
         {
            using (formTotpSetup setupDialog = new formTotpSetup())
               setupDialog.ShowDialog(this);
         };
         Controls.Add(linkTwoFactor);
         linkTwoFactor.BringToFront();
      }
      ...
      private bool Connect(Server server)
      {
         try
         {
            ...
            {
               // Two-factor authentication (TOTP), if configured.
               if (!TotpManager.VerifyInteractive(this))
               {
                  application = null;
                  return false;
               }
