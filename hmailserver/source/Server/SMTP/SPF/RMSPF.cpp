...
dnsquery(const char* domain, int type, DNS_RECORD** dnsrespp)
{
   ...
   *dnsrespp = NULL;
   ...
   // DNSSEC guard: TXT data with a bogus chain of trust is forged and
   // must not be used for SPF evaluation (implemented in DnssecResolver).
   if (type == DNS_TYPE_TEXT && HM::DnssecTxtLookupIsBogus(domain))
      return SPF_TempError;
