...
// rather than a constant, so that SpfVoidLookupLimit can still raise it or
// switch it off. The RFC's two is the default.
...
namespace HM
{
   ...
   SPFEvaluator::SPFEvaluator(std::shared_ptr<SPFDnsLookup> lookup) :
      ...
      terms_(0),
      void_terms_(0),
      void_term_limit_(MaximumVoidTerms),
   ...
   void
   SPFEvaluator::SetVoidTermLimit(int limit)
   {
      void_term_limit_ = limit;
   }
   ...
   {
      // Zero or less switches the limit off, which is what SpfVoidLookupLimit=0 has
      // meant since the setting was added. The RFC has no such case; an administrator
      // who has turned the limit off has said what they want, and the other limit of
      // section 4.6.4 - ten terms that query DNS - still bounds the evaluation.
      if (void_term_limit_ <= 0)
         return false;

      return void_terms_ > void_term_limit_;
   }
