...
//
// Changed in this tree: the void-term limit of section 4.6.4 is a member
// rather than a constant, so that SpfVoidLookupLimit - a setting this fork
// has shipped since 20 August 2026, and which the Control Panel offers -
// can still raise it or switch it off. The RFC's two is the default, and
// is what every test here uses.
...
namespace HM
{
   ...
   public:
      ...
      // Raises or switches off the void-term limit of section 4.6.4, which is
      // MaximumVoidTerms unless this is called. Zero or less means no limit at all -
      // the value this fork's SpfVoidLookupLimit setting has always used to mean that.
      // Nothing but SPF::Test calls it: the RFC's own number is what the conformance
      // suite and the testers measure against, and a bench whose answer depended on a
      // local setting would not be measuring the RFC.
      void SetVoidTermLimit(int limit);
