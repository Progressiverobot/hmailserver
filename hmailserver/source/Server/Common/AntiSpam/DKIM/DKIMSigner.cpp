...
namespace HM
{
   ...
   DKIMSigner::Sign(std::shared_ptr<Message> message)
   {
      ...
      // ARC sealing (RFC 8617): preserve authentication results in a signed
      // chain when relaying. Uses the same key material as DKIM.
      if (IniFileSettings::Instance()->GetArcSealingEnabled())
      {
         Arc arc;
         arc.Seal(message, domain, selector, privateKeyFile);
      }
   }
}