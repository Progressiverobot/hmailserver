...
namespace HM
{
   ...
   Utilities::GetHostNameFromReceivedHeader(const String &sReceivedHeader)
   {
      ...
      */
      ...
      if (endPos == -1)
      {
         // The header ends directly after the host name. A sender can produce
         // this, so it is a value to handle, not an assertion to trip.
   ...
   Utilities::GetIPAddressFromReceivedHeader(const String &sReceivedHeader)
   {
      ...
      */
      ...
      //
      // The client decides what goes in HELO/EHLO, so when both are present the
      // address the receiving server observed - the LAST one - is the one to
      // use. Taking the first bracket after "from", as this did until upstream
      // #546 was ported, let a sender choose the address the IP-based anti-spam
      // tests (DNSBL, SPF, HELO host) were run against.
      IPAddress result;
   ...
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Upstream issue #168 / commit b558cc7b8. The host name a client presents in
   // HELO/EHLO is copied verbatim into the "from" part of the Received header.
   // Since the sender decides what to put there, nothing in that part of the
   // header may decide which IP address the message is considered to originate
   // from. The address the server writing the header actually observed is the
