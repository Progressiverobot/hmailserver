...
namespace HM
{
   namespace
   {
      ...
      {
         // The listing's fields first, in its order, as the older tests hold;
         // the id last - the folder-permission routes name an account by it,
         // as the COM PermissionAccountID does, and the Linux suite's
         // Account.ID reads it by key.
         AnsiString entry;
         ...
         AnsiString id;
         id.Format(",\"id\":%I64d}", account->GetID());
         entry += id;
   ...
   RestApiServer::OpenApiRoutesPaths_()
   {
      ...
      return
         ...
         "\"get\":{\"summary\":\"Read an account\",\"description\":\"The account whole, as the update answers it: address and active first (the id, which a folder permission names an account by, comes last), then max_size_mb, first_name, last_name, forward_enabled, forward_address, forward_keep_original, signature_enabled, signature_plain_text, signature_html, vacation_enabled, vacation_subject, vacation_message, vacation_expires, vacation_expires_date, message_retention_days and admin_level - what InterfaceAccount reports over COM, less the password, the hash and the TOTP secret. Scoped to the address's domain.\",\"responses\":{\"200\":{\"description\":\"The account\"},\"404\":{\"description\":\"Unknown account\"}}},"
