...
// -----------------------------------------------------------------------------
// ssvscprintf: type-safe length counting for FormatV. The narrow overload must
// use _vscprintf — counting a narrow format string with the wide _vscwprintf
// (as the old FormatV did via a cast) miscomputes the length and makes the
// subsequent vsprintf_s fail-fast (CRT invalid parameter abort) when the
// guessed buffer is too small.
// -----------------------------------------------------------------------------

	inline int ssvscprintf(PCSTR pFmtA, va_list vl)
	{
      return _vscprintf(pFmtA, vl);
	}

	inline int ssvscprintf(PCWSTR pFmtW, va_list vl)
	{
      return _vscwprintf(pFmtW, vl);
	}
...
//					   this->begin(),
	...
	{
      // Count with the correct character width for this instantiation.
      // (The previous implementation cast the format string to wchar_t* and
      // always counted with the wide CRT function, which produced garbage
      // lengths for CStdStr<char> and could fail-fast in vsprintf_s.)
      int nLen = ssvscprintf(szFormat, argList);
      if (nLen < 0)
         return;
