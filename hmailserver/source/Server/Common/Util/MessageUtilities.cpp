...
namespace HM
{
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      for(String header : headers)
      {
         ...
         // header when the name can't be parsed meant continuing into the headers
         // below, which the sender may have written (upstream #168).
   ...
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Upstream issue #168 / commit b558cc7b8. The originating IP address is used
   // for the IP-based anti-spam tests (DNSBL, SPF, ...) when a message arrives
   // from an incoming relay or is downloaded from an external account. What the
   // sender presented in HELO/EHLO must not affect which address is picked - the
   // sender controls that string, and everything below the topmost Received
   // header the sender may have forged.
   //---------------------------------------------------------------------------()
   {
      std::list<String> headerLines;
      ...
      // The host name is what the sender claimed to be. It is used by the HELO
      ...
      // Without any header below it, the same message used to end up without a
      // known originating address, which silently disabled the IP-based spam
      // tests.
      headerLines.clear();
      headerLines.push_back("from my_pc (unknown [203.0.113.99]) by mail.example.test with ESMTP; Fri, 06 May 2016 03:49:14 +0200");
