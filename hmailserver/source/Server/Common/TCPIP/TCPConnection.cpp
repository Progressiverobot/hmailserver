...
namespace HM
{
   ...
   TCPConnection::AsyncHandshake()
   {
      ...
      // To do peer verification, it must either be enabled globally or required
      // for this specific connection (MTA-STS / DANE), and supported by the deriving class.
      bool enable_peer_verification = (Configuration::Instance()->GetVerifyRemoteSslCertificate() ||
                                       require_peer_verification_ ||
                                       !dane_records_.empty()) && IsClient();
      ...
      if (enable_peer_verification)
      {
         ...
         if (!dane_records_.empty())
         {
            // DANE-EE verification: match the remote certificate against the
            // published TLSA records rather than the PKIX chain.
            ssl_socket_.set_verify_callback(DaneVerifier(session_id_, dane_records_), error_code);
         ...
         else if (!expected_remote_hostname_.IsEmpty())
         {
