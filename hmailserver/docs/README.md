...
| [WarmStandby.md](WarmStandby.md) | Running a second machine that can take over when the first fails: share the database, replicate the data directory, keep the standby's service stopped, and know that DPAPI-protected passwords — route and fetch-account credentials, the per-domain relay password, private-key passphrases — do not travel between machines and have to be re-entered on the standby. |
...
| [Migration.md](Migration.md) | Bringing existing mailboxes into this server: from another IMAP server (an external account with the mirror on collects every folder, flags and dates kept), from mbox files or a Maildir with the Import Tool, from Outlook through IMAP, and from the upstream hMailServer in place. What each route keeps and what it does not, and why PST is read by Outlook rather than by us. |
...
| [RelocatingAnInstallation.md](RelocatingAnInstallation.md) | Moving a configured installation to a different directory or drive: every place the old path is recorded - the registry root in the 32-bit view, the service, 79 COM registrations, six `[Directories]` keys, eleven database columns - in the order they have to be repaired, and the two that stay broken. |
| [RegressionEnvironment.md](RegressionEnvironment.md) | Setting a machine up so `build/run-tests.ps1` passes: the service running from the repository build, SQL Server Compact and the bench database, ClamAV and SpamAssassin, the standard ports, and the environment traps that have broken runs before. `build/preflight-tests.ps1` is the machine-readable version of the same recipe; this is the human one. |
| [Fuzzing.md](Fuzzing.md) | Fuzzing the parsers with libFuzzer and AddressSanitizer: the harnesses under `fuzz/`, how to build and run them, the nightly job that runs all three and grows a corpus from night to night, the crash oracle the regression suite checks after every test, and what is covered (MIME, done) and what is not yet (HTML, iCalendar, vCard, Sieve, the SMTP command parser). |
...
| [WebmailShortcuts.md](WebmailShortcuts.md) | Using the webmail from the keyboard, and searching it: every shortcut the page answers to, and every operator the search box takes (`from:`, `has:attachment`, `larger:`, `older_than:`, `is:muted`, `category:`, `-word`, `OR` and the rest), with what each matches. |
| [ControlPanelDesign.md](ControlPanelDesign.md) | Changing or adding a page of the desktop Control Panel: the design system the pages are being moved onto since 15 September 2026 - the frame (command bar, sidebar and rail, breadcrumb), every token by name and value in the light, dark and High Contrast themes, the type ramp, the scaffold components with their public surface, the grid styles, the dialog frame, the rules (no new text without its seventeen translations, no implicit TextBlock style, colour plus shape plus word for every status) and the order a page is migrated in. |
| [DeckParity.md](DeckParity.md) | Asking how far the browser Control Deck and the REST API are from the desktop Control Panel: the census `build/check-deck-parity.py` takes of every field the desktop program writes, which route writes it and which Deck view reaches it, the gaps by interface, and what the heuristic gets wrong. The page itself speaks twenty languages beside English — the webmail's twenty that are read left to right, and not its Arabic, Hebrew and Persian — from the catalogues in `hmailserver/installation/WebAdmin/languages` that the packages install beside it and the server serves at `/deck-lang/<code>.json`; `build/check-deck-languages.py` holds every one of them to the page. |
...
| [CommandLine.md](CommandLine.md) | Administering the server from a shell or from PowerShell: `hmctl` (one Python 3 file, installed by the Linux packages as `/usr/bin/hmctl`) and the `HmailServer` PowerShell module, both over the REST API, so both work against a Windows server, a Linux server or one on another machine. The verbs, the credentials and why `--insecure` is refused off the loopback, accounts imported from and exported to a CSV file with a dry run, and what neither client does. |
...
| [Roadmap2.md](../../Roadmap2.md) | The programme after 6.3.2: what still stands between this server and the best there is, by who feels the gap, with what has landed since |
...
missing runbook is a real defect, and several of the documents above exist because
