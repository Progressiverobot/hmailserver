...
namespace RegressionTests.Stress.IMAP
{
   ...
   internal class ConcurrentConnectionsStressTests : TestFixtureBase
   {
      ...
      [Test]
      [Description(
         "Upstream #458: a session issues only FETCH, during which the server may not send EXPUNGE, so " +
      ...
      public void ConcurrentFetchAndExpungeNeverReturnsMismatchedBodies()
      {
         ...
         TestContext.WriteLine("Numbering race: {0} FETCHes, {1} EXPUNGEs over {2}.",
