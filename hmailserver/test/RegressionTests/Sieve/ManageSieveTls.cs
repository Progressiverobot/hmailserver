...
namespace RegressionTests.Sieve
{
   ...
   public class ManageSieveTls : TestFixtureBase
   {
      ...
      private sealed class Session : IDisposable
      {
         ...
         public void Dispose()
         {
            try { _sslStream?.Dispose(); } catch (Exception fatalCheck) when (!ExceptionPolicy.IsFatal(fatalCheck)) { /* Deliberately ignored: best effort only, and the outcome of the surrounding operation does not depend on this succeeding. */ }
            try { _networkStream?.Dispose(); } catch (Exception fatalCheck) when (!ExceptionPolicy.IsFatal(fatalCheck)) { /* Deliberately ignored: best effort only, and the outcome of the surrounding operation does not depend on this succeeding. */ }
