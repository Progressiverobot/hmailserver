...
{
   /// <summary>
   ///    Upstream #458: message sequence numbers must be stable within an IMAP session. When
   ///    another session expunges a message, this session's numbering may only change at points
   ///    where the server is allowed to send an untagged EXPUNGE (RFC 3501 7.4.1, RFC 2180
   ///    chapter 4). Before the per-session view every session numbered messages by their
   ///    position in the one collection the server holds per folder, so an expunge anywhere
   ///    renumbered everyone at once, and "FETCH 2 BODY[2]" returned message 3's attachment.
   /// </summary>
   [TestFixture]
   public class SessionMessageNumbering : TestFixtureBase
   {
      ...
      [Test]
      [Description("Upstream #458: FETCH of a body part returned another message's attachment after " +
      ...
      [Test]
      [Description("Upstream #458: the second half of the reported transcript - re-fetching the " +
      ...
      [Test]
      [Description("Upstream #458: fetching a message another session expunged must not return the " +
      ...
      [Test]
      [Description("Upstream #458, write side: STORE addressed by sequence number must not set flags " +
      ...
      [Test]
      [Description("Upstream #458, write side: COPY addressed by sequence number must never copy a " +
      ...
      [Test]
      [Description("Upstream #458: the reporter's client fetches by UID but then addresses body parts " +
      ...
      [Test]
      [Description("Upstream #458: POP3 DELE goes through FolderManager::DeleteInboxMessages, a second " +
                   "producer of delete notifications.")]
      ...
      [Test]
      [Description("Upstream #458: CLOSE deletes flagged messages without sending the closing " +
                   "session anything, so the other sessions have to be told some other way.")]
