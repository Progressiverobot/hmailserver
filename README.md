...
**Self-service portal** at `/portal` on the REST listener: a user signs in with the mailbox credential - the same checks as an IMAP logon, lockout and auto-ban included - and sees the quota, sets the automatic reply, changes the password (with the second-factor code when one is enrolled) under the password policy and reuse history an administrator is held to, and sees, releases or discards the messages held as suspected spam for that address - a release delivers to that address only, and a discard never touches another recipient's copy - and works the mailbox: the folder tree with its counts, a folder's newest messages, one message's text and attachment names, and on a message its flags, a move to another folder and a delete that goes to the Trash folder when there is one - all the account's own, and every IMAP session on the mailbox told of each change. It sends, too: a text message to any address the account could send to over SMTP, through the same delivery pipeline, with a copy kept in the Sent folder; attachments download as files; a search looks through subjects, senders and text, in one folder or all of them; folders shared with the account, public or another user's, sit in the same tree under the rights their owner granted; and the account's name, forwarding, signature and Sieve filter are its own to set. An HTML message is shown as it was sent, in a frame with an origin of its own where nothing runs and nothing is fetched; reply and forward prefill the form, a reply threads and marks the original answered, a draft is kept in the Drafts folder until it is sent, and the list is worked with the keyboard and acted on in bulk; files go out with a message as attachments, Forward and Edit draft keep the original's, and a large folder pages back. The endpoints behind it all live under `/api/v1/me` - the account itself, `password`, `vacation`, `quarantine`, `folders`, `messages` (with `flags`, `move` and `attachments`), `search`, `drafts`, `settings` and `filters` - and answer to an account's own credentials and to nothing else, and `POST /api/v1/session` turns the password into a browser session - an `HttpOnly`, `SameSite=Strict` cookie that ends after thirty idle minutes or twelve hours, whose changes must carry a header a browser never adds on its own; the page sends the password once, holds nothing in memory and allows no inline script.
