...
**Self-service portal** at `/portal` on the REST listener: a user signs in with the mailbox credential - the same checks as an IMAP logon, lockout and auto-ban included - and sees the quota, sets the automatic reply, changes the password (with the second-factor code when one is enrolled) under the password policy and reuse history an administrator is held to, and sees, releases or discards the messages held as suspected spam for that address - a release delivers to that address only, and a discard never touches another recipient's copy - and works the mailbox: the folder tree with its counts, a folder's newest messages, one message's text and attachment names, and on a message its flags, a move to another folder and a delete that goes to the Trash folder when there is one - all the account's own, and every IMAP session on the mailbox told of each change. Three endpoints behind it, `GET /api/v1/me`, `POST /api/v1/me/password` and `PUT /api/v1/me/vacation`, answer to an account's own credentials and to nothing else, and `POST /api/v1/session` turns the password into a browser session - an `HttpOnly`, `SameSite=Strict` cookie that ends after thirty idle minutes or twelve hours, whose changes must carry a header a browser never adds on its own; the page sends the password once, holds nothing in memory and allows no inline script.
