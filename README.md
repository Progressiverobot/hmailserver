...
* **Localised.** The Control Panel follows the Windows display language and ships in English and Swedish (Svenska); pick another from the sign-in card or the globe button in the sidebar footer, and it restarts in that language. A translation is one resx file, `Resources\Strings.<culture>.resx`, keyed by the English text; `build\check-localisation.py` keeps the English catalogue equal to the source and checks every language for lost placeholders and Alt keys. The shell, the sign-in, the navigation, the hand-written pages, every page, every dialog and the search palette are translated today, and the palette matches a query against the translated phrases; the verdict sentences composed in the service layer (the external-setup checks, the TLS, spam and virus verdicts, the certificate inspector, the key store) are English until their catalogue entries land (the script reports what is left).
