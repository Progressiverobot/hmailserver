...
**The Control Deck**, served at `/` by the same listener, is the browser front end for that API: a dashboard, the domains and their accounts, the delivery queue, TLSA records and the logs, and — since 9 September 2026 — editors that write. The three settings groups are drawn from the server's own OpenAPI document, so the page shows every setting the API accepts with its type, its permitted words and whether it takes effect on restart; a domain is created, edited in every field the desktop Control Panel's domain pages hold — limits, signature, relay, the out-of-office reply, DKIM and the other names it answers to — and deleted; rules, SMTP routes, certificates and listeners are created, edited and deleted, and a button restarts the services in place so a new listener takes effect. Every change is read back from the server after it is saved, and a refusal is shown in the server's own words beside the control that caused it. It keeps no password.
