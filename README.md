...
**[Download the latest release](https://github.com/Progressiverobot/hmailserver/releases/latest)** — a single `hMailServer-x.y.z-x64.exe` installer for Windows, and, since 6.3.0, a `.deb`, an `.rpm` and an AppImage for x86-64 and AArch64 Linux. Upgrading in place preserves your configuration and mail; the database upgrade chain is continuous from every earlier hMailServer version.
...
* **CardDAV** (RFC 6352): each account's address book - the one the webmail keeps - served to phones and desktop clients from the web services listener, found by RFC 6764 discovery. See [Administration](#administration).
...
* **SpamAssassin** integration, **DNSBL** and **SURBL** lookups (each server with an expected result, so a resolver's refusal code is never a listing), greylisting, HELO/PTR/MX sanity checks and a weighted scoring pipeline.
...
| Schema | Database version 6031, upgradeable from every earlier hMailServer release |
...
![The Control Panel's Welcome page](hmailserver/docs/images/control-panel-welcome.png)

**Self-service portal and webmail** at `/portal` on the REST listener: a user signs in with the mailbox credential - the same checks as an IMAP logon, lockout and auto-ban included - and gets a mail client in the browser and every account setting that is the account's own to set. Reading: the folder tree with its counts, a folder's newest messages paged back, conversations grouped by thread, one message with its text, its HTML shown as the server's own document under a policy of its own (nothing runs, nothing is fetched until the reader allows it for the message or the sender), attachments downloaded or previewed, the raw headers with the SPF, DKIM and DMARC verdicts and a warning on a link whose text and target disagree, labels as IMAP keywords with colours and a `label:` search, a search as a reader writes it (`from:`, `to:`, `subject:`, `has:attachment`, `before:`, `after:`, `in:`, `is:unread`, quoted phrases), keyboard shortcuts (`?` lists them), bulk actions, Archive, Junk and Trash by designation, and Empty this folder. Writing: a rich-text editor whose HTML is rebuilt from an allowed subset and sent beside its text, a From picker over the account's identities (its own address, its aliases, and addresses whose owners granted it the post right), a signature, templates with `{first_name}`-style fields, undo send, drafts, reply and forward that keep the original's attachments, read receipts asked for and answered (RFC 8098), one-click unsubscribe (RFC 8058) to a public address only, send later and snooze until a time, a file above the domain's threshold sent as a link that expires and can take a password, and S/MIME in the browser - sign, verify, encrypt and decrypt with the Web Crypto API, the private key wrapped with the account's password before the server stores it. The pages: Settings (name, signature, theme, density, notifications, language - the Control Panel's seventeen and, since 12 September 2026, Greek, Korean and European Portuguese, beside English), Away (out of office, forwarding, the addresses that reach the mailbox), Filters (a rules editor that writes Sieve, or the script by hand), Contacts (kept as the account writes, completing the To field), Storage (the quota, every folder's size, the largest messages, a clean-up of Trash and Junk by age), Files (the links sent, their downloads, their expiry), Security (app passwords made only with the account's own password proven again, browser sessions ended one at a time or all but this one, support access an administrator may be allowed for a while, the S/MIME key store), and the messages held as suspected spam, released or discarded for that address only. The page installs as an app, keeps the inbox listing and the last thirty opened messages for reading without a connection, queues a send that fails for want of one, carries the server's branding and announcement, imports `.eml` files and exports a folder as mbox, and is worked with a screen reader: landmarks, a skip link, focus that follows a change of address. The endpoints behind it all live under `/api/v1/me` - the account itself, `password`, `vacation`, `quarantine`, `folders`, `messages` (with `flags`, `move`, `html`, `attachments`, `receipt`, `unsubscribe` and `snooze`), `search`, `drafts` (with `schedule`), `settings`, `preferences`, `identities`, `contacts`, `filters`, `app-passwords`, `sessions`, `storage`, `files`, `scheduled` and `smime` - and answer to an account's own credentials and to nothing else; `POST /api/v1/session` turns the password into a browser session - an `HttpOnly`, `SameSite=Strict` cookie that ends after thirty idle minutes or twelve hours, whose changes must carry a header a browser never adds on its own; the page sends the password once, holds nothing in memory and allows no inline script. The full description of every route is in the OpenAPI document at `/api/v1/openapi.json`; the keyboard shortcuts and the search operators are listed in [WebmailShortcuts.md](hmailserver/docs/WebmailShortcuts.md).

**CardDAV** (RFC 6352) on the web services listener, so a phone or a desktop client syncs the account's contacts - the same address book the webmail keeps. A client given the mail address finds it by discovery: `/.well-known/carddav` redirects to `/dav/`, a PROPFIND there answers the principal, the principal answers the address-book home, and the home holds one address book, `Contacts`, at `/dav/addressbooks/<address>/contacts/`. HTTP Basic as the account (the password or an app password; the IMAP logon's lockout and auto-ban apply), over HTTPS only - over plain HTTP the answer is 403 with the reason, unless a TLS-terminating proxy in front says `X-Forwarded-Proto: https`. PROPFIND at depth 0 and 1, the `addressbook-multiget` and `addressbook-query` reports, `sync-collection` with a whole-collection token (an empty token lists everything, the current one answers no changes, a stale one is refused with `DAV:valid-sync-token` so the client lists again - the store keeps no change log), strong ETags and `getctag`, and GET, PUT and DELETE of one contact as vCard 3.0 with `If-Match` and `If-None-Match`; vCard 3.0 and 4.0 are accepted on PUT. The store is a name and one e-mail address per contact, and the endpoint says so rather than pretending otherwise: a card without an `EMAIL` is refused with `CARDDAV:valid-address-data` and a sentence, and a card's phone numbers, postal addresses, organisation, notes and photo are not kept. The store has no column for a client's resource name or UID either, so a contact created under a client's name is answered `201` with the server's name (`<id>.vcf`) in `Location` and a UID derived from the row; a client that honours `Location` keeps one copy, and one that does not sees its copy replaced by the server's at its next listing. `CardDavRedirectUrl` still points the well-known URL at another server when set.

![The webmail: the inbox](hmailserver/docs/images/webmail-inbox.png)

![The webmail: a message open](hmailserver/docs/images/webmail-message.png)

**REST administration API** for domains, accounts, aliases, distribution lists, SMTP routes, the server's settings, rules, certificates and listeners, the delivery queue, the quarantine, the archive, server status and TLSA records, with authenticated access and bounded request handling (a size cap and a receive deadline, so a slow or oversized request cannot occupy a worker). Callers authenticate with the administrator password or with a **scoped API key** — a bearer token stored only as a SHA-256 digest, with a mandatory expiry, an optional source-address restriction, and a scope that is read-only unless you say otherwise and can be confined to named domains. No key of any scope can mint or revoke keys; that needs the administrator password, or a narrow key would escalate itself in one request. A browser signs in once instead: `POST /api/v1/session` exchanges the administrator password (and a one-time code, where one is enrolled) for an `HttpOnly`, `SameSite=Strict` session cookie that ends when the administrator password changes, and every write it makes must carry `X-Requested-With`.

**The Control Deck**, served at `/` by the same listener, is the browser front end for that API: a dashboard, the domains and their accounts, the delivery queue, TLSA records and the logs, and — since 9 September 2026 — editors that write. The three settings groups are drawn from the server's own OpenAPI document, so the page shows every setting the API accepts with its type, its permitted words and whether it takes effect on restart; rules, SMTP routes, certificates and listeners are created, edited and deleted, and a button restarts the services in place so a new listener takes effect. It keeps no password.
...
| `/adminpassword=<password>` | **Custom to this installer.** Sets the administrator password (at least 5 characters) without the wizard page, and forwards it to the database tool so a silent upgrade of an installation that already has a password can authenticate |
...
* **A fresh silent install leaves the administrator password unset unless you pass `/adminpassword=<password>`.** The wizard page that collects it is skipped, so without the switch the database tool runs without a password. Set it afterwards — through the Control Panel, or via the COM API (`Application.Authenticate` then `Settings.SetAdministratorPassword`).
* **A silent install of an existing installation still needs the password** for the database upgrade, and cannot prompt for it: pass `/adminpassword=<password>`. Without it the database upgrade fails and the installer reports a non-zero exit code rather than waiting (until 6.2.23 it hung on a password dialog nobody was there to answer).
...
Web services (MTA-STS hosting, client autoconfiguration, CardDAV):

   <pre>
   WebServicesHttpPort=0         ; 80 to enable
   WebServicesHttpsPort=0        ; 443 to enable (uses the ACME certificate if none is set); CardDAV needs this one
   ...
   CalDavRedirectUrl=            ; /.well-known/caldav redirects here (RFC 6764); empty = 404, this server has no CalDAV
