...
**Self-service portal** at `/portal` on the REST listener: a user signs in with the mailbox credential - the same checks as an IMAP logon, lockout and auto-ban included - and sees the quota, sets the automatic reply and changes the password (with the second-factor code when one is enrolled), under the password policy and reuse history an administrator is held to. Three endpoints behind it, `GET /api/v1/me`, `POST /api/v1/me/password` and `PUT /api/v1/me/vacation`, answer to an account's own credentials and to nothing else, and `POST /api/v1/session` turns the password into a browser session - an `HttpOnly`, `SameSite=Strict` cookie that ends after thirty idle minutes or twelve hours, whose changes must carry a header a browser never adds on its own; the page sends the password once, holds nothing in memory and allows no inline script.
