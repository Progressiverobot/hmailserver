...
| Sub-folder | Purpose |
|---|---|
| `AntiSpam/` | Anti-spam algorithms: SPF validation, SURBL lookups, DNS blacklists, greylisting, score-based filtering, DMARC evaluation (`DMARC/`), and DKIM/ARC (`DKIM/` — RSA + Ed25519 signing/verification, ARC sealing in `Arc.{h,cpp}`) |
| `AntiVirus/` | Anti-virus software integrations (Custom scanner, ClamAV scanner)|
| `Application/` | Application startup, configuration loading, scheduling, and logging. `IniFileSettings` holds all `hMailServer.INI` settings; `Application::StartServers` starts the optional metrics/REST/web-services listeners |
...
| `Util/` | Miscellaneous utilities. Includes the optional service listeners: `MetricsServer` (Prometheus), `RestApiServer` (REST admin API), `WebServicesServer` (MTA-STS hosting/autoconfig/autodiscover), `AcmeClient` (Let's Encrypt), `TlsRptStore` (TLS-RPT aggregation) |
...
Outbound transport security lives here: `TlsPolicy.{h,cpp}` implements MTA-STS (RFC 8461) policy discovery/caching and DANE TLSA lookups (RFC 7672, DNSSEC-validated via `Common/TCPIP/DnssecResolver`); `ExternalDelivery.cpp` applies the per-host requirements (required TLS, certificate pinning, bogus-chain host exclusion) and feeds `TlsRptStore`; `TlsRptReporterTask` sends the daily RFC 8460 reports. The vendored SPF implementation is in `SPF/RMSPF.cpp`.

### `Server/IMAP/`

IMAP4rev1 server implementation including IDLE, SORT, Quota, ACL, MOVE (RFC 6851), ID (RFC 2971) and SPECIAL-USE (RFC 6154) extensions. Each IMAP command has a corresponding command-handler class. Folder hierarchy and message flags are stored in the database via the Persistence layer.
...
`Administrator/` talks exclusively to the server through the COM API (`Server/COM/`). `DBSetup` and `DBUpdater` also go through the COM API. No tool accesses the database directly. One exception: the "Server features" dialog (`Administrator/Dialogs/formServerFeatures.cs`) edits `hMailServer.INI` directly (located via the registry), because those settings are INI-based rather than database-based.

Notable Administrator additions: a live dashboard (`Main panes/ucDashboard.cs`), optional TOTP two-factor authentication for the GUI logon (`Utilities/TwoFactorAuth.cs`), and the Server features dialog for all 6.0 INI settings.
...
**Database portability.** The SQL layer abstracts over MySQL, MS SQL Server, and PostgreSQL. Use parameterised queries exclusively; never construct SQL strings manually.
...
**Implementation history:** `IMPLEMENTATION-NOTES.md` at the repository root is the detailed log of the 6.0 modernization — per-feature implementation details, OpenSSL 4.0 API gotchas, CStdStr conventions, third-party library build recipes, and the new-machine setup checklist. Read it before modifying any 6.0 feature.

**Build-event warning:** the server vcxproj's pre/post build events stop and re-register the production Windows service. On a machine running a production hMailServer, always build with `/p:PreBuildEventUseInBuild=false /p:PostBuildEventUseInBuild=false` (the build scripts below do this automatically). The compiler runs with `/WX`; new code must compile without warnings.
